...
async def download_attachments_b64(session: aiohttp.ClientSession, attachments: list) -> list:
    ...
    for att in attachments:
        ...
        if content_type and not content_type.startswith("image/"):
            print(f"skip non-image attachment: {filename} ({content_type})", flush=True)
            continue
        if not url:
        ...
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                if resp.status != 200:
                    ...
                    continue
                raw = await resp.read()
                compressed, mime = await asyncio.get_event_loop().run_in_executor(None, compress_image, raw)
                ...
                print(f"downloaded attachment: {filename} -> {out_name}", flush=True)
        except Exception as exc:
...
async def on_ready():
    ...
    print("Channel filtering is disabled: all non-bot messages are forwarded to n8n", flush=True)
...
async def on_message(message: discord.Message):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            print(f"POST n8n channel_id={payload['body']['body']['channel_id']} images={len(json.loads(payload['body']['body']['attachments_b64']))}", flush=True)
            async with session.post(N8N_URL, json=payload, timeout=aiohttp.ClientTimeout(total=180)) as resp:
                body = await resp.text()
                if resp.status != 200:
                    print(f"n8n error {resp.status}: {body[:500]}", flush=True)
                else:
